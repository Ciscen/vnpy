...
月度滚动（walk-forward）训练流水线：
  ...
  4. 拼接所有月度信号，返回完整 signal DataFrame
...
from hs300_top10.features.labeler import generate_weekly_labels
...
def rolling_train(
    ...
    for i, (m_start, m_end) in enumerate(months):
        ...
        # 训练截止日期 = 当月第一天的前一天
        from datetime import date, timedelta
        train_cutoff = (date.fromisoformat(m_start) - timedelta(days=1)).isoformat()

        # 训练集：截止到 train_cutoff 的历史数据（最多 train_years 年）
        from datetime import date as _date
        train_start_limit = _date.fromisoformat(train_cutoff) - timedelta(days=train_years * 365)
