...
# 验证输出
ls hs300_top10/output/v1.3/
# 应该看到: dashboard.html, stats.json, trades.csv 等
```
...
# 6.1 测试飞书文档读取
python -c "
from hs300_top10.live.portfolio import load_portfolio_from_feishu
...
python -m hs300_top10.run_live --force-run --dry-run

# 6.3 测试飞书推送（实际发送卡片）
python -m hs300_top10.run_live --force-run
...
30 8 * * 1-5 cd /opt/vnpy && source .env && .venv/bin/python -m hs300_top10.run_live >> hs300_top10/live/logs/live.log 2>&1

# 每月 1 号 7:00 强制重训模型
0 7 1 * * cd /opt/vnpy && source .env && .venv/bin/python -m hs300_top10.run_live --retrain >> hs300_top10/live/logs/retrain.log 2>&1
```
...
```bash
# 创建日志目录
mkdir -p /opt/vnpy/hs300_top10/live/logs

# 配置 logrotate（可选，防止日志过大）
cat > /etc/logrotate.d/hs300 << 'EOF'
/opt/vnpy/hs300_top10/live/logs/*.log {
    weekly
    rotate 12
    compress
    missingok
...
```bash
python -m hs300_top10.run_live [OPTIONS]
