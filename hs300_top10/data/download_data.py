# hs300_top10/data/download_data.py
"""
hs300_top10/data/download_data.py

功能
----
1. 使用 akshare 获取当前 **沪深 300** 成分股列表（暂不考虑历史动态成分），以避免 akshare 接口不支持 `trade_date` 参数的问题。
2. 基于该成分股列表下载过去 10 年（2016‑01‑01 ~ 2026‑04‑30）的日线 OHLCV 数据。
3. 实现本地缓存（Parquet）并支持增量下载，避免重复请求导致 IP 被封。
4. 解决时间格式、重复数据以及防封禁的细节问题。
"""
