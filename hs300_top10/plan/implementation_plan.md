...
5. **代码组织**：所有新增代码统一放在项目根目录下的新文件夹 `hs300_top10` 中，保持与原项目结构相互独立。
...
> 5. **模型持久化路径** – 默认保存至 `hs300_top10/models/`，文件名依据模型自动命名（如 `lasso.pkl`）。如有其他需求请说明。
...
> [!WARNING]
> - **数据获取**：需要从公开数据源（如 TuShare、Wind、Yahoo）下载 HS300 成分股最近 10 年（2016‑01‑01 至 2026‑04‑30）的日线 OHLCV，并保存为 `hs300_top10/data/hs300.csv`。请确认是否使用 TuShare（需 API token）或其他渠道。
...
> - **代码目录**：所有新建文件将放在 `hs300_top10/` 目录下，包括 `data/`, `features/`, `model/`, `strategy/`, `backtest/` 子文件夹。
...
│
├─ strategy/
│   │   hs300_top10_strategy.py   # vnpy 策略实现（选股、止盈、止损）
│
├─ backtest/
...
#### 7. `strategy/hs300_top10_strategy.py`
